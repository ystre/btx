...
static btx_result_t next_token(lexer_t *l, token_t *tok) {
    ...
    if (l->p + 1 < l->end && l->p[0] == '\\' && l->p[1] == 'b') {
        ...
        while (pos < 8 && l->p < l->end) {
            char c = *l->p;
            if (c == '\'' && pos == 4) { l->p++; continue; }
            if (c == '0')      { tok->bits[pos++] = 0; l->p++; }
            else if (c == '1') { tok->bits[pos++] = 1; l->p++; }
            else if (c == '_') { tok->bits[pos++] = 2; l->p++; }
            else break;
        }
...
static btx_result_t parse(const char *text, size_t len, buf_t *out) {
    lexer_t    l;
    token_t    tok;
    bitstate_t bs = {0, 0, 0};
    btx_result_t r;
    ...
    for (;;) {
        ...
        if (bs.next_bit != 0) return BTX_ERR_BIT_INCOMPLETE;
